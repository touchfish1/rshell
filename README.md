...
- 首页主机管理已统一纳管 `SSH/Telnet/Zookeeper/Redis`，支持同入口连接、编辑、删除
- 新增主机支持直接选择 `Redis` 协议，并支持测试连接/保存并连接
- Redis 页增强：Key 搜索（默认模糊匹配）、树状分组、DB 切换、命令日志、可调整布局
- Redis 二进制兼容进一步提升：支持 JDK 序列化内容自动识别与可读展示（key 与 value）
- 审计能力扩展到 `Zookeeper/Redis` 关键操作，并新增审计报表视图（事件/主机/趋势统计）
- UX 优化：删除确认、未保存关闭确认、错误可见化、关键文案 i18n 与可访问性改进
...
- 支持获取已保存密码并在编辑界面按需显示
- Redis 与 Zookeeper 连接支持在首页统一管理（连接/编辑/删除）
