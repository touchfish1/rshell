...
- 新增 MySQL 客户端能力：连接管理、库/表浏览、表数据查询与筛选
- MySQL 数据区支持按列筛选、横向滚动与固定表头，提升大表查看体验
- 页面结构解耦：`Terminal / Zookeeper / Redis / MySQL` 样式拆分为独立文件，减少互相污染
- MySQL 页面交互优化：双击库/表生成浏览 tab，数据区与导航区联动更清晰
- 持续完善 UI 细节与稳定性（滚动区域、表头显示、提示文案）
