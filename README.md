...
- 审计能力扩展到 MySQL：连接、断开、库表浏览、SQL 执行与 EXPLAIN、表结构变更等操作均可追踪
- 审计弹窗增强：新增 MySQL 类型筛选，报表中新增 MySQL 事件统计卡片
- 审计多语言文案补齐：中英文筛选项与报表指标同步更新
- 保持已有首页统一连接管理与 MySQL 结构编辑体验，继续加强跨模块一致性
...
- MySQL 审计：连接、断开、数据库/表/字段查询、SQL 执行与 EXPLAIN、表结构变更
